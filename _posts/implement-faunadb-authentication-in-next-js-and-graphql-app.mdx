...
coverImage: /assets/blog/nextjs-faunadb-authentication/cover.jpeg
publishDate: '2020-09-08T06:55:08.132Z'
...
The `CurrentIdentity` function returns the ref of the document associated with the `token`, and the `Get` function uses the ref to return the corresponding document. We also need the `id`, so include it in the response data.
