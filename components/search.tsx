  ...
  return (
    <div ref={ref} className="relative">
      ...
      {active && results && results.length > 0 && (
        <ul className="absolute sm:right-0 sm:w-max max-h-screen/2 overflow-y-auto mt-1 py-2 bg-white dark:bg-gray-700 border border-gray-200 dark:border-gray-600 rounded-md shadow-lg z-10">
          {results.map(({ slug, title }: Results) => (
            <li
              key={slug}
              className="px-4 py-2 text-sm dark:text-gray-100 leading-5 text-left cursor-pointer hover:bg-gray-100 dark:hover:bg-purple-700"
            >
              <CustomLink href={`/blog/${slug}`} onClick={handleClickInside}>
                {title}
              </CustomLink>
            </li>
          ))}
